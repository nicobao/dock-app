...
import {AccountDetailsScreen, filterTransactionHistory} from './AccountDetailsScreen';
...
describe('AccountDetailsScreen', () => {
  ...
  describe('expect to filter transaction history items', () => {
    ...
    it('expect to hide received transactions with failed status', () => {
      const transactions = filterTransactionHistory([txReceived, txReceivedFailed], address1);
      expect(transactions.length).toBe(1);
    });

    it('expect to add sent flag', () => {
      const transactions = filterTransactionHistory([txReceived, txSent], address1);
    ...
    it('expect to filter only transactions for the given address', () => {
      const transactions = filterTransactionHistory([txOtherAddress, txSent], address1);
      expect(transactions.length).toBe(1);
    });
  });
});
